...
var zy = require('ditzzsenpai-api')
var {
	Searchnabi,
	Gempa
} = require('./../lib');
...
router.get('/canvas/welcome', async(req, res, next) => {
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(listkey.includes(apikey)){
  let hasil = `https://hadi-api.herokuapp.com/api/card/welcome3?username=${username}&memberCount=${memberCount}&bg=${bg}&pp=${pp}&gcicon=${gcicon}&gcname=${gcname}`
  data = await fetch(hasil).then(v => v.buffer())
...
router.get('/canvas/goodbye', async(req, res, next) => {
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(listkey.includes(apikey)){
  let hasil = `https://hadi-api.herokuapp.com/api/card/goodbye3?username=${username}&memberCount=${memberCount}&bg=${bg}&pp=${pp}&gcicon=${gcicon}&gcname=${gcname}`
  data = await fetch(hasil).then(v => v.buffer())
         await fs.writeFileSync(__path +'/tmp/welcome.png', data)
        res.sendFile(__path+'/tmp/welcome.png')
  } else {
